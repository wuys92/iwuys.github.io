...
//商品属性点击过滤查询  yandk_add_20150109
function clickProdArr(attrName,attrVal){
	var v_prodAttrs = "{}";//属性jsong字符串
	if(attrName!=""){
	  v_prodAttrs = "{'"+attrName+"':'"+attrVal+"'}";
	  $("#attr_"+attrName).val(attrVal);
	}else{
	  $("#attr_"+attrName).val("");//
	}
	//alert(v_prodAttrs)
	$("#prodAttrs").val(v_prodAttrs);
	var jvForm = getJvForm();
	jvForm.onsubmit=null;
	jvForm.submit();
}
...
function showMore(a,d){
	if(jQuery("#FacetBrandsMoreBtn")[0]&&jQuery("#FacetBrandsBackBtn")[0]){
		if(d&&d=="more"){
						jQuery(".hideBrandNode").show('fast');
			      jQuery("#FacetBrandsMoreBtn").hide();
            jQuery("#FacetBrandsBackBtn").show();            
        }else{
        	jQuery(".hideBrandNode").hide('fast');
        	jQuery("#FacetBrandsMoreBtn").show();
        	jQuery("#FacetBrandsBackBtn").hide();       	
        }
    }
