	...
	if (d.size() > 0) {
		...
		if (jQuery("#perOrderQuotaNum").size() > 0) {
			...
			if (e != null) {
				e = parseInt(e);
				if (e > 0 && b > e) {
					alert("该商品每单限购 [" + e + "] 件，请修改购物数量,若给您带来不便，我们深表歉意。");
					b = e;
				}
			}
		}
...
function posttoWb(url, f, e) {
	...
	var a = encodeURI;
	var b = "";
	var d = url + "?title=" + f + "&url=" + c + "&appkey=" + e + "&site=" + a
			+ "&pic=" + b;
	window
			.open(
					d,
...
function showOriginalPicture() {
	jQuery("#detailPictureSlider li").each(
			function() {
				if (jQuery(this).find("img").attr("original")) {
					jQuery(this).find("img").attr("src",
							jQuery(this).find("img").attr("original"));
				}
			});
}
...
/* 商品小图片切换 */
jQuery("#productImgA").bind("click", function(d) {
	...
	$.fn.jqzoom = function(options) {
		...
		return this
				.each(function() {
					...
					var loader = null;
					loader = new Loader();
					var ZoomTitle = (jQuery.trim(aTitle).length > 0) ? aTitle
							: (jQuery.trim(imageTitle).length > 0) ? imageTitle
									: null;
					...
					Lens.prototype.setposition = function(e) {
						...
						if (firstime == 0) {
							var lensleft = (smallimagedata.w) / 2
									- (lensdata.w) / 2;
							var lenstop = (smallimagedata.h) / 2 - (lensdata.h)
									/ 2;
							$("div.jqZoomPup").show();
							if (settings.lens) {
						...
						if (overleft()) {
							...
							if (overright()) {
								if ($.browser.msie) {
									lensleft = smallimagedata.w - lensdata.w
											+ bleft + 1
								} else {
									lensleft = smallimagedata.w - lensdata.w
											+ bleft - 1
						...
						if (overtop()) {
							...
							if (overbottom()) {
								if ($.browser.msie) {
									lenstop = smallimagedata.h - lensdata.h
											+ btop + 1
								} else {
									lenstop = smallimagedata.h - lensdata.h - 1
											+ btop
								}
							}
						}
						...
						function overleft() {
							return mousepos.x - (lensdata.w + 2 * 1) / 2
									- bleft < smallimagedata.pos.l
						}
						function overright() {
					...
					Lens.prototype.findborder = function() {
						...
						lensbleft = (lensbleft.length > 0) ? eval(lensbleft)
								: 0
					};
					...
					Largeimage.prototype.setposition = function() {
						this.node.style.left = Math.ceil(-scale.x
								* parseInt(lens.getoffset().left) + bleft)
								+ "px";
						this.node.style.top = Math.ceil(-scale.y
								* parseInt(lens.getoffset().top) + btop)
								+ "px"
					};
					Largeimage.prototype.setinner = function(e) {
						this.node.style.left = Math.ceil(-scale.x
								* Math.abs(e.pageX - smallimagedata.pos.l))
								+ "px";
						this.node.style.top = Math.ceil(-scale.y
								* Math.abs(e.pageY - smallimagedata.pos.t))
								+ "px";
						$("div.jqZoomPup img", a).css({
							position : "absolute",
					...
					Largeimage.prototype.setcenter = function() {
						this.node.style.left = Math.ceil(-scale.x
								* Math.abs((smallimagedata.w) / 2))
								+ "px";
						this.node.style.top = Math.ceil(-scale.y
								* Math.abs((smallimagedata.h) / 2))
								+ "px";
						$("div.jqZoomPup img", a).css({
							position : "absolute",
							top : this.node.style.top,
					...
					function Stage() {
						...
						case "right":
							leftpos = (smallimagedata.pos.r
									...
									+ smallimagedata.w + Math
									.abs(settings.xOffset))
									: (smallimagedata.pos.l
											- settings.zoomWidth - Math
											.abs(settings.xOffset));
						...
						case "top":
							...
							leftwindow = smallimagedata.pos.l
									+ settings.xOffset + settings.zoomWidth;
					...
					function zoomTitle() {
						this.node = jQuery("<div />").addClass("jqZoomTitle")
								.html("" + ZoomTitle + "");
					...
					function Loader() {
						...
						$(this.node).appendTo("body").css("visibility",
								"hidden");
						this.show = function() {
							switch (settings.preloadPosition) {
							case "center":
								loadertop = smallimagedata.pos.t
										+ (smallimagedata.h - $(this.node)
												.height()) / 2;
								loaderleft = smallimagedata.pos.l
										+ (smallimagedata.w - $(this.node)
												.width()) / 2;
								break;
							default:
